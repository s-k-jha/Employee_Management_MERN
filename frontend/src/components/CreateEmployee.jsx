...
const CreateEmployee = () => {
    ...
    let formHandle = (e) => {
        ...
        if (!name || !email || !phone || !designation || !gender || !course || !image) {
            alert("For Successfull registration Fill all the required fields..!")
        }
        else {
            axios.post("http://localhost:4001/employees", payload, {
                ...
                .then((e) => { alert(e.data) })
                .catch(() => { console.log("can not registered"); })
        ...
        return (
            <div className=' max-w-[940px]  h-[600px] border-4 border-blue-900 mx-auto relative top-[-80px] shadow-xl scale-75 p-[30px]'>
                ...
                <div className='border border-red-600 max-w-[300px] mx-auto my-5 p-8'>
                    ...
                    <select onChange={(e) => { setDesignation(e.target.value); }} name='designation' required class="block appearance-auto w-full bg-white border border-gray-400 hover:border-gray-600 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline">
                    ...
                    <button className='bg-blue-300 ml-14 rounded-lg p-2' onClick={formHandle}>Register Me</button>
                </div>
